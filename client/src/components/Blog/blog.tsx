...
export default function Blog(props : Props) {
    const [likedPosts,
        setLikedPosts] = useState < LikedPost[] > ([]);
    const [isFormVisible,
        setIsFormVisible] = useState < boolean > (false);
    const [isHide,
        setIsHide] = useState < boolean > (false);
    const [title,
        setTitle] = useState < string > ("");
    const [content,
        setContent] = useState < string > ("");
    const [blog,
        setBlog] = useState < BlogContent[] > ([]);
    const [editingPostId,
        setEditingPostId] = useState < string | null > (null);
    const [likedList,
        setLikedList] = useState < LikedList[] > ([]);
    const [isLikeList,
        setIsLikeList] = useState < boolean > (false);
    const [currentPostId,
        setCurrentPostId] = useState < string > ("")
    const [isUpdating,
        setIsUpdating ]= useState<boolean>(false);

    useEffect(() => {
        setIsFormVisible(props.isNewPost);
    }, [props.isNewPost]);

        // if user is login-out while updating a form //
        useEffect(() => {
            if (isUpdating && props.isLoginOutWhileUpdating) {
                handleCloseWhileUpdate();
            }
    ...
    function handleLikes(postId : string) {
        ...
        if (userData.id && userName.length > 0) {
            ...
            if (!alreadyLiked) {
                ...
                setBlog(prevBlog => prevBlog.map(post => post.id === postId
                    ? {
                        ...post,
                        likes: post.likes + 1
                    }
    ...
    function handleUpdate(event? : FormEvent) {
        ...
        const updatedBlog = blog.map(post => post.id === editingPostId
            ? {
                ...post,
                title,
                content
    ...
    function handleCloseWhileUpdate(event?: FormEvent){
        if (event){
        event.preventDefault();
    }
        setIsHide(false);
    ...
    const hide = isHide
        ? "hide"
        : "";
    const overlay = isLikeList
        ? "overlay"
        : "";
    const hideLikeList = isLikeList
        ? ""
        : "hide";
            ...
            {blog.map(post => (
                ...
                <div className={`${hide} div-post`} key={post.id}>
                    ...
                    <button
                        ...
                        {post.likes > 1
                            ? (` ${post.likes} persons likes this post `)
                            : (post.likes === 0
                                ? ("")
                    ...
                    <div className={`${overlay} ${hideLikeList}`}>
                        <div className={`like-list ${hideLikeList}`}>
                            ...
                            <button
                                ...
                                type="button"
                                onClick={() => handleLikeList(post.id)}>X</button>

                            <ul className="ul-no-style ">
                                <li className="text-center">Likes</li>
                                <li><hr/></li>
                                {likedList
                                    .filter(user => user.postId === currentPostId)
                                    .length > 0
                                    ? (likedList.filter(user => user.postId === currentPostId).map((user) => (
                                        <li className=" flex-row inline-block" key={user.id}>
                                            <div className=" flex flex-row ">
                                                ...
                                                <div className="  flex-row margin-likes-name ">
                                                    <p className="admin-name truncate">
                                                        {user.name}
                                                    </p>
                                                    <p className="truncate">
                                                        there will be some presentation .
                                                    </p>
                                                </div>
